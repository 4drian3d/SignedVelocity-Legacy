package io.github._4drian3d.signedvelocity.legacy.bukkit.listener;

import com.google.common.io.ByteArrayDataInput;
import com.google.common.io.ByteStreams;
import io.github._4drian3d.signedvelocity.legacy.bukkit.SignedVelocity;
import io.github._4drian3d.signedvelocity.legacy.common.SignedQueue;
import io.github._4drian3d.signedvelocity.legacy.common.SignedResult;
import org.bukkit.entity.Player;

import java.util.Objects;
import java.util.UUID;

public class PluginMessageListener implements org.bukkit.plugin.messaging.PluginMessageListener {
  private final SignedVelocity plugin;

  public PluginMessageListener(final SignedVelocity plugin) {
    this.plugin = plugin;
  }

  @Override
  public void onPluginMessageReceived(String channel, Player player, byte[] message) {
    if (!Objects.equals(channel, SignedVelocity.CHANNEL)) {
      return;
    }
    @SuppressWarnings("UnstableApiUsage")
    final ByteArrayDataInput input = ByteStreams.newDataInput(message);

    final UUID playerId = UUID.fromString(input.readUTF());
    final String source = input.readUTF();
    final String result = input.readUTF();
    final SignedQueue queue;
   switch (source) {
     case "COMMAND_RESULT": queue = plugin.getCommandQueue(); break;
     case "CHAT_RESULT": queue = plugin.getChatQueue(); break;
     default : throw new IllegalArgumentException("Invalid source " + source);
    };
    final SignedResult resulted;
    switch (result) {
      case "CANCEL": resulted = SignedResult.cancel(); break;
      case "MODIFY" : resulted = SignedResult.modify(input.readUTF()); break;
      case "ALLOWED": resulted = SignedResult.allowed(); break;
      default: throw new IllegalArgumentException("Invalid result " + result);
    };
    queue.dataFrom(playerId).complete(resulted);
  }
}
